...
class Play():
    ...
    def det_Pose_Video(self, user_video, target_video, play_id, option=True):
        # DB_host = "127.0.0.1"
        # DB_port = 27017
        ...
        RESULT_BOX = {}
        RESULT_BOX_2 = {}
        # clip_1 = mp.VideoFileClip(user_video)
        # clip_1.audio.write_audiofile("wav1.wav")
        # clip_1 = mp.VideoFileClip(target_video)
        # clip_1.audio.write_audiofile("wav2.wav")

        
        x1, sr = librosa.load("wav1.wav")
        x2, sr = librosa.load("wav2.wav")
        
        hop_size = 2205
        n_fft = 4410

        x1_chroma = librosa.feature.chroma_stft(y=x1, sr=sr, tuning=0, norm=2, hop_length=hop_size, n_fft=n_fft)
        x2_chroma = librosa.feature.chroma_stft(y=x2, sr=sr, tuning=0, norm=2, hop_length=hop_size, n_fft=n_fft)

        D, wp = librosa.sequence.dtw(x1_chroma, x2_chroma, subseq=True)
        wp_s = np.asarray(wp) * int(hop_size) / sr
        ...
        clip2 = clip2.subclip(result["start"], result["end"])

        final_clip = clips_array([[clip1,clip2]])

        # 3. 영상 파일을 불러오기
        cap = cv2.VideoCapture(final_clip)
...
if __name__ == '__main__':
    ...
    # 영상 경로
    VIDEO_1 = "sample_data/sample.mp4" # 유저 업로드 영상
    VIDEO_2 = "sample_data/BTS-Dynamite1-3.mp4"
    ...
    play.det_Pose_Video(user_video=VIDEO_1,target_video=VIDEO_2, play_id="123")